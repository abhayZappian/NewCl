...
const EspTable = () => {
  ...
  const getData = async () => {
    ...
    const { results } = await getAllEspProviderAccount();
    setData(results);
    setEspDetails(results);
  ...
  const handleAction = async (id, actionType, message) => {
    ...
    await ESPAction(id, actionType, message);
    setTimeout(() => {
      getData(currentPage);
    }, 300);
  };
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="left" style={{ ...headerCellStyle }}>
                    Email Service Provider <span style={dividerStyle}></span>
                  </TableCell>
                  ...
                  itemsToDisplay?.map((row, index) => (
                    <>
                      <CustomTableRow key={row.esp_account_id} index={index}>
                        ...
                        <TableCell>{row.from_domain}</TableCell>
