...
const EspTable = () => {
  ...
  const getData = async () => {
    ...
    const results = await axiosInstance.get(apiEndPoints.getalldisaster);
    setData(results.data.result);
    setEspDetails(results.data.result);
      ...
      case action === "0":
        if (id) {
          handleAction(
            espName.id,
            0,
            "Disaster Type In-Active Successfully !!!"
          );
        }
        break;
      case action === "1":
        if (id) {
          handleAction(espName.id, 1, "Disaster Type Active Successfully !!!");
        }
        break;
      default:
        if (id) {
          handleAction(espName.id, 2, "Disaster Type Deleted Successfully !!!");
        }
    }
  ...
  return (
    <>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="left" style={{ ...headerCellStyle }}>
                    Action to be taken <span style={dividerStyle}></span>
                  </TableCell>
                  ...
                  itemsToDisplay?.map((row, index) => (
                    <>
                      <CustomTableRow key={row.esp_account_id} index={index}>
                        <TableCell>{row?.data?.disaster?.disaster}</TableCell>
                        <TableCell>
                          {row?.data?.actionType?.actionValue}
                        </TableCell>
                          ...
                          {row.data.actions === "Active" ? (
                            <LightTooltip title="Active">
                              <IconButton
                                onClick={() => {
                                  setEspName({
                                    name: row.data.disaster.disaster,
                                    id: row.data._id,
                                  });
      ...
      <ConfirmationDialog
        ...
        handleClose={action}
        title={`${espName.name}`}
        content="Are you sure you want to Delete this Disaster Type?"
      ...
      <ConfirmationDialog
        ...
        handleClose={action}
        title={`${espName.name}`}
        content="Are you sure you want to In-active this Disaster Type?"
      ...
      <ConfirmationDialog
        ...
        handleClose={action}
        title={`${espName.name}`}
        content="Are you sure you want to Active this Disaster Type?"
